...
package org.openbakery.examples.console;
...
public class Main implements InSimListener {
    ...
    public void run() {
        ...
        try {
            ...
            client.send(new MessageRequest("/?"));
            
            client.send(new MessageRequest(MessageRequest.RED + "Red, " + MessageRequest.WHITE + "White and  " + MessageRequest.BLUE + "Blue"));
            
            client.send(new SmallRequest(Small.START_SENDING_GAUGES, 1));

            
            Thread.sleep(5000); // Sleep for 5 seconds to let a few OutGauge packets come through.
